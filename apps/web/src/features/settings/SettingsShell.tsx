  ...
  return (
    <div className="space-y-4 rounded-2xl border border-border/70 bg-card/60 p-6">
      <div>
        ...
        <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
          Riset internet real-time (Tavily) untuk AI. Setelah API key tersimpan, AI otomatis melakukan riset mendalam — beberapa
          pencarian bertahap dari sudut pandang berbeda — kapan pun membutuhkan informasi terkini atau pengetahuan di luar router.
          Hasilnya tampil sebagai kartu sumber (judul + tautan) di chat. Tidak ada tombol aktif/nonaktif terpisah; hapus API key
          di bawah bila ingin menonaktifkan.
        </p>
      </div>
      <p className="text-sm">
        Status: {status?.configured ? <span className="font-medium text-emerald-600">Aktif (key tersimpan)</span> : <span className="text-muted-foreground">Belum dikonfigurasi</span>}
      </p>
      <div className="space-y-2">
        <label htmlFor="tavily-key" className="text-xs font-medium">API key Tavily</label>
