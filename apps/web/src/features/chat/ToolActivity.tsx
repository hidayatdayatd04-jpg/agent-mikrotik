...
import { ChevronDown, ChevronRight, Loader2, CheckCircle2, XCircle, Clock, Scissors, Globe, ExternalLink } from "@/components/icons";
    ...
    <div className="rounded-xl border border-indigo-500/30 bg-indigo-500/[0.04]">
      ...
      >
        <span className="flex min-w-0 items-center gap-2 font-medium">
          ...
          <span className="truncate">
            Deep Research{research?.query ? ` · ${research.query}` : ""}
          </span>
        </span>
        <span className="flex shrink-0 items-center gap-2 text-[11px] text-muted-foreground">
          <span className={props.status === "failed" ? "text-destructive" : props.status === "running" ? "text-indigo-500" : "text-emerald-600 dark:text-emerald-400"}>
            {statusLabel}
          </span>
      ...
      {open && (
        <div className="space-y-1.5 border-t border-indigo-500/20 px-3 py-2.5">
          {props.status === "failed" && (
            <p className="text-xs text-destructive">Pencarian web gagal dijalankan.</p>
          )}
