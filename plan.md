...
- [ ] Implementasikan penghapusan draft/orphan, retensi objek chat terhapus, dan retry cleanup; tidak ada objek user lain yang ikut terhapus. (DELETE /api/attachments/files/:id draft terbukti E2E: objek hilang dari B2 (listing verifikasi), row hilang; POST /api/attachments/cleanup sweep orphan >24h milik user sendiri; retensi objek saat chat dihapus: FK cascade menghapus row DB tapi objek B2 masih ada — gap-open ditandai untuk M10 sweep berjadwal)
