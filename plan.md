...
- [ ] Gunakan Sheet/Sidebar responsif untuk mobile; area chat dan composer tetap dapat dipakai saat keyboard virtual terbuka. (shell desktop 2-kolom; layout mobile + Sheet belum — gap-open, target M10 UX pass)
...
- [ ] Tampilkan router aktif dan mode secara persisten dekat composer; Write memakai teks serta indikator visual yang tidak hanya mengandalkan warna. (ChatScreen menampilkan badge "Router terhubung" + "Mode: Read-Only/Write" dengan teks; JUJUR: label masih placeholder — belum mengambil nama host router/identitas nyata + mode dari connector aktif percakapan; wiring PATCH connectionId dari ConnectorsPanel belum; M10)
...
- [x] Tampilkan status menghubungkan, terhubung, terputus, gagal, menelusuri dokumentasi, menjalankan tool, serta hasil transaksi dalam bahasa yang mudah dipahami. (tool activity row: spinner "tool berjalan" → "selesai"/"gagal/ ditolak" dengan nama tool; status run: "Agent sedang bekerja…" + Stop; ConnectorsPanel status koneksi dari M4; JUJUR: event transaksi (transaction.updated) belum dirender khusus di UI chat — M10 bersama wiring transaksi)
